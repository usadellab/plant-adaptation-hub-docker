...
export default function PerformMDS() {
  ...
  const [mdsStudies, setMdsStudies] = useState([])

  const [selectedMdsStudy, setSelectedMdsStudy] = useState("");
  ...
  useEffect(() => {
    if (studies.length > 0) {
      ...
      setSelectedMdsStudy(first_study.study_identifier);
...
if(selectedMdsStudy){
  axios
  .post(`${apiEndpoint}/assays?studyId=${selectedMdsStudy}&token=${token.apiToken}`)
  .then((response) => {
...
const saveSelectedAssays = async (checked) => {
  ...
  const disMethodsList = [
    ...
    const handleSelectedMdsStudy = (studyId) => {
      setSelectedMdsStudy(studyId)
    ...
    const handleSelection = (selectedItems) => {
      ...
      handleSelectedMdsStudy(studyId);
...
<Typography variant="p">
  {`This tool computes and visualizes MDS coordinates based on the same genotypic data available for GWAS analyisis.`} For more details please see <b><a target="blind" href="https://ataulhaleem.github.io/camelina-hub-documentation/modules/Stratification/mds">documentation</a></b>
</Typography>
...
{/* <Grid sx={{ mt: 2 }}>
          ...
          defaultValue={studies[0].study_title}
            ...
            options={mdsStudies.map((study) => study.study_title)}
            renderInput={(params) => (
              <TextField {...params} label="Study Title" 
              sx={{
                '& .MuiOutlinedInput-root': {
                  '& fieldset': {
                    borderColor: selectedMdsStudy ? 'green' : 'red',
                  },
                },
              }}
              />
...
<SearchGrid  studies={mdsStudies} onSelection={handleSelection}></SearchGrid>

        <Grid container direction='row' spacing={2} columns={2} sx={{ padding: 1 }}>
