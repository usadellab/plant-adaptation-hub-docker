...
//                 `<br>` +
//                 `<b style="color: #2980b9;">Geographic Origin:</b> <span style="color: #2c3e50;">${d.biological_material_country}</span><br>` +
...
const GeoLocator = ({ studyId, width = '100vw', height = '100vh', scale = 2, type = 'mercator' ,       center = {
  lat: 40,
  lon: 50,
},
  ...
  // const [plotLayout, setPlotLayout] = useState({
  //   showlegend: false,
  //   width: width,  // Dynamically set width from props
  //   height: height, // Dynamically set height from props
  //   geo: {
  //     showcountries: true,
  //     showland: true,
  //     showocean: true,
  ...
  const [plotLayout, setPlotLayout] = useState({
    ...
    geo: {
      ...
      showland: true,
      showocean: true,
      oceancolor: '#87CEFA', //"#cce5ff", // Light blue ocean color
      showcoastlines: true,
      coastlinecolor: '#d3d3d3',
      landcolor: '#eddbad', //'#e0e0e0',
      projection: {
        type: type,
        scale: scale,
      },
      bgcolor: "#f9f9f9", // Same as ocean color
      domain: {
        x: [0, 1],
        y: [0, 1],
      },
    ...
    plot_bgcolor: "#f9f9f9", // Background color of the plot
    paper_bgcolor: "#f9f9f9", // Background color of the entire canvas
    margin: {
      l: 0,
      r: 0,
      t: 0,
  ...
  return (
    <Box sx={{ width: '100vh', backGroundColor :'#F9F9F9' }}      
> 
      {!germplasmData.length > 0 || (
        <Plot
          data={[
